...
### Group by Education
diabetes %>% group_by(Education) %>% summarize(mean(Diabetes_binary), sd(Diabetes_binary))

### Group by Sex
diabetes %>% group_by(Sex) %>% summarize(mean(Diabetes_binary), sd(Diabetes_binary))

### Contingency Tables
table(diabetes$Diabetes_binary, diabetes$Education)
table(diabetes$Diabetes_binary, diabetes$Income)

table(diabetes$Education, diabetes$Income)

### Histogram BMI/Diabetes

g <- ggplot(diabetes, aes(x=BMI))
g + geom_histogram(aes(fill = as.factor(diabetes$Diabetes_binary), y = ..density..), position = "dodge") +
