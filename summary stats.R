...
mean(diabetes_e$Diabetes_binary)

### Group by Education
diabetes %>% group_by(Age) %>% summarize(mean(Diabetes_binary), sd(Diabetes_binary))

### Group by Sex
diabetes_e %>% group_by(Sex) %>% summarize(mean(Diabetes_binary), sd(Diabetes_binary))

### Contingency Tables
table(diabetes_e$Diabetes_binary, diabetes_e$Income)

### Histogram BMI/Diabetes

g <- ggplot(diabetes_e, aes(x=BMI))
...
g <- ggplot(diabetes_e, aes(x = Income))
...
correlation <- cor(select(diabetes_e, Diabetes_binary, HighBP, HighChol, CholCheck, Stroke, HeartDiseaseorAttack, PhysActivity, Fruits, Veggies, HvyAlcoholConsump, AnyHealthcare, NoDocbcCost, DiffWalk, m_hlth, p_hlth), method = "spearman")
